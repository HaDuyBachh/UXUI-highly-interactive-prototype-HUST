...
namespace E2C.ChartBuilder
{
    public class BarChartBuilder : E2ChartBuilder
    {
        ...
        protected override void CreateItems()
        {
            ...
            //category background
            if (options.chartStyles.barChart.enableCategoryBackground)
            {
                int div = dataInfo.zoomMax - dataInfo.zoomMin + 1;

                E2ChartGraphicBarChartBar background = E2ChartBuilderUtility.CreateEmptyRect("Category Background", dataRect, true).gameObject.AddComponent<E2ChartGraphicBarChartBar>();
                background.transform.SetAsFirstSibling();
                background.maskable = false;
                if (options.rectOptions.inverted)
                {
                    background.rectTransform.offsetMin = new Vector2(-hAxis.minPadding, 0.0f);
                    background.rectTransform.offsetMax = new Vector2(hAxis.maxPadding, 0.0f);
                }
                else
                {
                    background.rectTransform.offsetMin = new Vector2(0.0f, -vAxis.minPadding);
                    background.rectTransform.offsetMax = new Vector2(0.0f, vAxis.maxPadding);
                }
                background.color = options.chartStyles.barChart.categoryBackgroundColor;
                background.width = Mathf.Clamp(options.chartStyles.barChart.categoryBackgroundWidth, 0.0f, grid.xAxis.unitLength);
                background.barOffset = 0.0f;
                ...
                if (batchCount > 0)
                {
                    ...
                    {
                        E2ChartGraphicBarChartBar batchItem = GameObject.Instantiate(background, dataRect);
                        batchItem.transform.SetAsFirstSibling();
            ...
            if (options.chartStyles.barChart.enableBarBackground)
            {
                ...
                for (int i = 0; i < num; ++i)
                {
                    ...
                    E2ChartGraphicBarChartBar background = E2ChartBuilderUtility.CreateEmptyRect("Bar Background", seriesRect[i], true).gameObject.AddComponent<E2ChartGraphicBarChartBar>();
            ...
            for (int i = 0; i < dataInfo.seriesCount; ++i)
            {
                ...
                if (options.plotOptions.colorMode == E2ChartOptions.ColorMode.ByData) bar.barColors = colorList.ToArray();
                bar.width = barWidth;
        ...
        protected override void CreateLabels()
        {
            ...
            int interval = options.label.interval >= 1 ? options.label.interval : 1;
            for (int i = 0; i < dataInfo.seriesCount; ++i)
            {
                if (dataInfo.activeDataCount[i] == 0) continue;
                ...
                for (int j = dataInfo.zoomMin; j <= dataInfo.zoomMax; j += interval)
                {
        ...
        protected override void CreateCustomLabels()
        {
            ...
            for (int i = 0; i < customLabels.labelInfo.Length; ++i)
            {
                ...
                {
                    float offsetLabel = info.labelOffset * Mathf.Sign(barList[info.seriesIndex].dataValue[info.dataIndex]);
                ...
                {
                    float offsetIcon = info.iconOffset * Mathf.Sign(barList[info.seriesIndex].dataValue[info.dataIndex]);
